...
- Lazily initialized by test code in `tests/specs/modes/file_walking.rs`
...
- Contains `self_loop -> .` symlink pointing to itself (committed in git)
...
- **Static**: Committed files (most fixtures)
- **Lazy**: Generated automatically by tests when needed (bench-deep, JS dependencies)

No manual setup is required - just run `cargo test`.
