...
impl TestsCheck {
    ...
    fn run_branch_scope(
        ...
        result
            .without_tests
            .retain(|path| !should_skip_path(path, allow_placeholders, diff_range, ctx.root));
        ...
        finalize_correlation_result(violations, &config.check, metrics, self.name())
    }
    ...
    fn run_commit_scope(
        ...
        for commit in &commits {
            ...
            for path in &analysis.source_without_tests {
                // Skip if has inline tests or placeholders
                if should_skip_path(
                    path,
                    allow_placeholders,
...
// =============================================================================
// Correlation Checking Helpers
// =============================================================================

/// Check if a source file should be skipped from "missing tests" violations
/// due to inline tests or placeholder tests.
fn should_skip_path(
    path: &Path,
    ...
    // Check for placeholder tests if allowed
    if allow_placeholders && has_placeholder_for_source(path, root) {
        return true;
    }
...
/// Finalize correlation check result based on violations and config.
